...
def tail_logs(lines: int = 20, date: Optional[str] = None) -> list[dict]:
    ...
    scan_id = None
    if date:
        scan_id = _find_latest_scan_for_date(date)

    entries = store.get_scan_logs(scan_id=scan_id, tail=lines)
    return entries
...
def query_logs(date: Optional[str] = None,
    ...
    entries = store.get_scan_logs(
        scan_id=scan_id, category=category, level=level, code=code, tail=tail,
    )
    return entries
...
def clear_logs(days: int = 7) -> dict:
    ...
    cleared = store.delete_old_scan_logs(cutoff_ts)
    return {"cleared": cleared, "kept": -1, "files_removed": []}


def list_logs() -> list[dict]:
    """列出所有扫描的日志概要"""
    folders = store.list_scan_log_folders()
    return folders
...
def log_summary(date: Optional[str] = None) -> dict:
    ...
    return store.log_summary(scan_id=scan_id)


def _find_latest_scan_for_date(date: str) -> Optional[str]:
    """按日期前缀查找最新的 scan_id"""
    folders = store.list_scan_log_folders()
    for f in folders:
        if f["scan_id"].startswith(date):
            return f["scan_id"]
    return None
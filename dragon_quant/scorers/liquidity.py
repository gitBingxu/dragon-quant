...
from dragon_quant.scorers.base import at_limit, clip, desc_rank_score
...
def score(code: str, cache: DataCache, primary_sector: str = "",
    ...
    # 相对分：板块成分股换手降序排名分位
    peer_codes = {s.code for s in components if s.code in qmap}
    if q:
        peer_codes.add(code)
    peer_to = [qmap[c].turnover_rate for c in peer_codes]
    s_to_rel = desc_rank_score(to, peer_to)[0]
    degraded = len(peer_to) <= 1 or len(peer_codes) < len({s.code for s in components})
    ...
    if is_limit_up and at_limit(q.bid1_price, q.limit_up) and q.bid1_volume > 0 and q.volume > 0:
        ...
        s_seal_strength = R.SEAL_NEUTRAL
        degraded = True
        reasons.append("涨停但有效封单盘口缺失")
    else:
        s_seal_strength = 0.0
        if not q:
            degraded = True
            reasons.append("个股行情缺失")
    if minute and q and q.limit_up > 0:
        n_open = _count_open(minute, q.limit_up)
        touched = any(at_limit(bar.high, q.limit_up) for bar in minute)
        s_seal_stable = (R.SEAL_STABLE_SCORES[0 if n_open == 0 else 1 if n_open <= 2 else 2]
                         if touched else 0.0)
    else:
        n_open = -1
        s_seal_stable = R.SEAL_NEUTRAL
        degraded = True
        reasons.append("分时或涨停价缺失")
    ...
    return ScoreResult(
        ...
        details={
            ...
            "reasons": reasons, "turnover_sample_size": len(peer_to),
        },
    )
...
def _count_open(minute: list[KBar], limit_up: float) -> int:
    ...
    for bar in sorted(minute, key=lambda b: b.timestamp):
        at_close = at_limit(bar.close, limit_up)
        touched = at_limit(bar.high, limit_up)
        if not at_close and (sealed or touched):
            opens += 1
        sealed = at_close
    return opens

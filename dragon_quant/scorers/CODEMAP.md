...
| `absorption.py:89` | 六根连续 K 检测、每板块独立因果校验，记录原始窗口起止时间 |
| `absorption.py:156` | 正向强度 60% + 广度 20% + 持续性 20%；拉升参考 3%、出逃规模参考 10 |
| `absorption.py:42` | 同时段重叠窗口合并，最强代表按三交易日半衰期向 50 衰减，取衰减后最强三事件均值 |
...
- 承接窗口按同时段时间重叠合并（含链式重叠），次数本身不加分；事件按目标历史交易日年龄衰减，最终取最强三事件均值。`raw_event_count` 与 `event_count` 分别表示窗口与独立事件。
- 逐板块先跌后拉只是时序证据，不证明实际资金流；行业级评分不对同板块个股虚构差异。
